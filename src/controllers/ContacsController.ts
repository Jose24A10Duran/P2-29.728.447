...
import { ContactsModel } from "../models/ContacsModel";

export class ContacsController {
    ...
    static  async add(req: Request, res: Response) {
        ...
        try {
            ...
            if (result.success) {
                res.render("contact", { message: "Tu mensaje ha sido enviado con exito" });
            } else {
                res.status(500).render("contact", { message: "Error al guardar el mensaje." });
            }
        } catch (error) {
            console.error("Error en ContacsController.add", error);
